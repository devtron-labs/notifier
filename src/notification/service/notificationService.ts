...
class NotificationService {
    ...
    public sendNotification(event: Event) {
       this.logger.info("inside the sendNotification function",event)
        ...
        findByEventSource(event.pipelineType, event.pipelineId, event.eventTypeId, event.appId, event.envId, event.teamId).then((settingsResults) => {
          this.logger.info('notificationSettingsRepository.findByEventSource')
